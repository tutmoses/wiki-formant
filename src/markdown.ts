...
//
// Lived in its own `entities.ts` with no subpath of its own, so the only way to
// reach `decodeEntities` was the root barrel — and the README claimed it came
// from here, which is where it now actually is.
